...
const Index = () => {
  ...
  const bookSections = useMemo(() => {
    ...
    baseBooks.forEach((book) => {
      ...
      if (!authorGroups[authorKey]) authorGroups[authorKey] = [];
    ...
    return { collectionGroups, popularSeries, bestsellers, bestAuthors, topSelling, newArrivals };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-20 md:pt-24">
        ...
        <main className="container mx-auto px-4 py-12 scroll-mt-32" id="books-section">
          ...
          <div className="text-center mb-12 animate-fade-in">
            ...
            <p className="text-muted-foreground max-w-2xl mx-auto">
              Browse our carefully curated collection of literary treasures. From timeless classics to modern bestsellers — find your next great read.
            </p>
          ...
          {!searchQuery && selectedCategory !== "Collection" && filteredAndSortedBooks.length > 0 && (
            <div className="animate-fade-in space-y-4">
              {bookSections.newArrivals.length > 0 && (
                <BookSection title="✨ New Arrivals" books={bookSections.newArrivals} onAddToCart={handleAddToCart} icon="clock" variant="new" />
              )}
              {bookSections.popularSeries.length > 0 && (
                <BookSection title="📖 Popular Series" books={bookSections.popularSeries} onAddToCart={handleAddToCart} icon="sparkles" variant="featured" />
              )}
              {bookSections.bestsellers.length > 0 && (
                <BookSection title="🏆 Bestsellers" books={bookSections.bestsellers} onAddToCart={handleAddToCart} icon="trophy" variant="default" />
              )}
              {bookSections.bestAuthors.length > 0 && (
                <BookSection title="⭐ Best Authors" books={bookSections.bestAuthors} onAddToCart={handleAddToCart} icon="star" variant="featured" />
              )}
              {bookSections.topSelling.length > 0 && (
                <BookSection title="📈 Top Selling" books={bookSections.topSelling} onAddToCart={handleAddToCart} icon="trending" variant="default" />
              )}
            </div>
          )}
        ...
        {/* Footer */}
        <footer className="relative bg-gradient-to-b from-card to-muted/30 border-t border-border mt-16 overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-primary via-secondary to-primary" />
          
          <div className="absolute inset-0 pointer-events-none">
            <Sparkles className="absolute top-8 left-[10%] h-4 w-4 text-secondary/30 animate-sparkle" />
            <Sparkles className="absolute top-12 right-[15%] h-3 w-3 text-primary/30 animate-sparkle" style={{ animationDelay: "0.5s" }} />
            <Library className="absolute bottom-20 left-[20%] h-5 w-5 text-primary/20 animate-gentle-float" />
          </div>
          ...
          <div className="container mx-auto px-4 py-12 relative z-10">
            ...
            <div className="mt-8 pt-8 border-t border-border text-center">
              <div className="flex items-center justify-center gap-2 mb-2">
                ...
                <span className="font-playfair text-lg text-vintage-gradient font-bold">Happy Reading!</span>
